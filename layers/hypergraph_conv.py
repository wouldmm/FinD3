...
class HypergraphConv(MessagePassing):
    ...
    def batch_scatter(src, index, batch_offsets, dim_size):
        out = torch.zeros(dim_size, device=src.device)
        for info in batch_offsets:
            mask = (index >= info["edge_offset"]) & (index < info["edge_offset"] + info["num_edges"])
            local_index = index[mask] - info["edge_offset"]
            out_batch = scatter(src[mask], local_index, dim=0, dim_size=info["num_nodes"])
            out[info["node_offset"]:info["node_offset"] + info["num_nodes"]] += out_batch
        return out